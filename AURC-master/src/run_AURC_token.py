...
def main():
    ...
    n_existing_token_pt_files = len([filename for filename in os.listdir(args.output_dir) if re.search(task+"_token_\d\d\d.pt",filename)])
    n_existing_config_json_files = len([filename for filename in os.listdir(args.output_dir+"configs/") if re.search(task+"_config_\d\d\d.json",filename)])
    n_existing_predictions_dev_json_files = len([filename for filename in os.listdir(args.output_dir+"store_predictions_here/") if re.search(task+"_predictions_dev_\d\d\d.json",filename)])
    n_existing_predictions_test_json_files = len([filename for filename in os.listdir(args.output_dir+"store_predictions_here/") if re.search(task+"_predictions_test_\d\d\d.json",filename)])
    ...
    MODEL_PATH = os.path.join(args.output_dir,'{}_token_{}.pt'.format(task,pad_to_3_digits(n_existing_token_pt_files)))
    print(MODEL_PATH)
    CONFIG_PATH = os.path.join(args.output_dir,'configs/{}_config_{}.json'.format(task,pad_to_3_digits(n_existing_config_json_files)))
    print(CONFIG_PATH)
    PREDICTIONS_DEV = os.path.join(args.output_dir,'store_predictions_here/{}_predictions_dev_{}.json'.format(task,pad_to_3_digits(n_existing_predictions_dev_json_files)))
    print(PREDICTIONS_DEV)
    PREDICTIONS_TEST = os.path.join(args.output_dir,'store_predictions_here/{}_predictions_test_{}.json'.format(task,pad_to_3_digits(n_existing_predictions_test_json_files)))
   ...
   # TRAIN DATA
    ...
    if args.train:
        ...
        # Main Loop
        print("##### SETUP: token-wise, DOMAIN:", args.target_domain, ",", "use CRF:", args.crf, ",", "learning-rate:", args.learning_rate, ",", "DROPOUT:", config.hidden_dropout_prob)
        print()
